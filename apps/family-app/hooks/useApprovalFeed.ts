...
export function useApprovalFeed(): {
  ...
  const query = useQuery({
    queryKey: queryKeys.approvalsFeed(parentId),
    queryFn: () => fetchApprovals(parentId, token).then((response) => response.items),
    enabled: USE_REAL_API && Boolean(token) && Boolean(parentId),
  });

  useEffect(() => {
    if (query.isError) {
      console.error('Failed to load approvals feed', query.error);
    }
  }, [query.error, query.isError]);

  return {
    items: USE_REAL_API ? query.data ?? [] : dummyItems,
